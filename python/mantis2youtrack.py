...
def to_yt_issue(mantis_issue, value_sets, names_to_ignore):
    issue = Issue()
...
def get_yt_values_from_mantis_values(yt_field_name, mantis_values):
    ...
    if yt_field_name not in mantis.FIELD_VALUES:
        return [mv.replace("/", " ") for mv in mantis_values]
...
def add_values_to_fields(connection, project_id, mantis_field_name, values, mantis_value_to_yt_value):
    ...
    for field_name in get_yt_cf_name_from_mantis_cf_name(mantis_field_name):
        ...
        if hasattr(pcf, "bundle"):
            ...
            yt_values = [v for v in [mantis_value_to_yt_value(value, bundle, value_mapping) for value in values] if len(v.name)]
...
def mantis2youtrack(target_url, target_login, target_pass, mantis_db_name, mantis_db_host, mantis_db_port,
    ...
    value_sets.update(
        create_auto_attached_bundle_custom_fields(target, u"resolution", mantis.RESOLUTION_VALUES.values(),
    ...
    for name in mantis_project_names:
        ...
        try:
            ...
            target.createProjectDetailed(str(project_id), name, client.get_project_description(project_id),
                target_login)
        ...
        for attachment in mantis_attachments:
            ...
            target.createAttachment("%s-%s" % (project_id, attachment.bug_id), attachment.filename, content, authorLogin
